...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Data Unit Type</h1>
    </div>
        ...
        <div class="col-xl-12 col-md-12 col-lg-12 mb-1">
            <div class="card shadow mb-4">
                ...
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    <h6 class="m-0 font-weight-bold text-primary">Data Unit Type</h6>
                </div>
                <!-- Card Body -->
                <div class="card-body">
                    <div class="row float-right mr-0 mb-2">
                        ...
                        <button class="btn btn-sm btn-info" data-toggle="modal" data-target="#addUnitTypeModal">
                            <i class="fas fa-fw fa-plus-circle"></i> Add Unit Type</button>
                    </div>
                    <div class="text-xs">
                        <table class="table unit-types-datatable display" style="width:100%;">
                        ...
                        <input type="hidden" id="urllist" name="urllist" value="{{url('master/unit-types/list')}}">
                        <input type="hidden" id="urldel" name="urldel" value="{{url('master/unit-types')}}">                        
                    </div>
                </div>
            </div>
        </div>
    ...
    @include('Admin.SUPER.unit-types.modal.add')
    @include('Admin.SUPER.unit-types.modal.edit')
...
<script>
    ...
    var table = $('.unit-types-datatable').DataTable({   
        pageLength : 25,
        dom: 'lfrtip',        
        processing: true,
        serverSide: true,
    ...
    function deleteUnitType(params) {
        Swal.fire({
            title: 'Yakin?',
            text: "Anda akan menghapus Data Unit Type.",
            icon: 'warning',
    ...
    function clearEditFormUnitType() {
    ...
    function saveUnitType() {
        ...
        if (name=="") {
            Swal.fire({icon: 'error', title: 'Oops...',text: "Unit Type Name Required!",});
        }
        else{
            let datar = {};
            datar['_method']='POST';
    ...
    function updateUnitType() {
        ...
        if (name=="") {
            Swal.fire({icon: 'error', title: 'Oops...',text: "Unit Type Required!",});
        }
        else{
            let datar = {};
            datar['_method']='POST';
    ...
    $(document).on('click', '.editUnitTypeBtn', function () {
        // Ambil data dari atribut tombol
        var id = $(this).data('id');
        var name = $(this).data('name');

        // Set data ke form dalam modal
        $('#editUnitTypeModal input[id="e_id"]').val(id);
        $('#editUnitTypeModal input[id="e_name"]').val(name);
    });
</script>
@endsection