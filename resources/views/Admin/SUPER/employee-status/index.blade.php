...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Data Employee Status</h1>
    </div>
        ...
        <div class="col-xl-12 col-md-12 col-lg-12 mb-1">
            <div class="card shadow mb-4">
                ...
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    <h6 class="m-0 font-weight-bold text-primary">Data Employee Status</h6>
                </div>
                <!-- Card Body -->
                <div class="card-body">
                    <div class="row float-right mr-0 mb-2">
                        ...
                        <button class="btn btn-sm btn-info" data-toggle="modal" data-target="#addEmployeeStatusModal">
                            <i class="fas fa-fw fa-plus-circle"></i> Add Employee Status</button>
                    </div>
                    <div class="text-xs">
                        <table class="table employee-status-datatable display" style="width:100%;">
                        ...
                        <input type="hidden" id="urllist" name="urllist" value="{{url('master/employee-status/list')}}">
                        <input type="hidden" id="urldel" name="urldel" value="{{url('master/employee-status')}}">                        
                    </div>
                </div>
            </div>
        </div>
    ...
    @include('Admin.SUPER.employee-status.modal.add')
    @include('Admin.SUPER.employee-status.modal.edit')
...
<script>
    ...
    var table = $('.employee-status-datatable').DataTable({   
        pageLength : 25,
        dom: 'lfrtip',        
        processing: true,
        serverSide: true,
    ...
    function deleteEmployeeStatus(params) {
        Swal.fire({
            title: 'Yakin?',
            text: "Anda akan menghapus Data Employee Status.",
            icon: 'warning',
    ...
    // Form Sort Function
    function clearFormEmployeeStatus() {
        document.getElementById('status_name').value='';
    } 

    function clearEditFormEmployeeStatus() {
        document.getElementById('e_id').value='';
        document.getElementById('e_status_name').value='';
    } 

    function saveEmployeeStatus() {
        let status_name = $('input[id=status_name').val();
        
        if (status_name=="") {
            Swal.fire({icon: 'error', title: 'Oops...',text: "Status Name Required!",});
        }
        else{
            let datar = {};
            datar['_method']='POST';
            datar['_token']=$('._token').data('token');
            datar['status_name']=status_name;
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            ...
            success: function(data) {
                if (data.error==false) {
                    table.ajax.reload();
                    clearFormEmployeeStatus();
                    $('#addEmployeeStatusModal').modal('hide');
    ...
    function updateEmployeeStatus() {
        ...
        let status_name = $('input[id=e_status_name').val();        
    ...
    $(document).on('click', '.editEmployeeStatusBtn', function () {
        // Ambil data dari atribut tombol
        var id = $(this).data('id');
        var status_name = $(this).data('status_name');

        // Set data ke form dalam modal
        $('#editEmployeeStatusModal input[id="e_id"]').val(id);
        $('#editEmployeeStatusModal input[id="e_status_name"]').val(status_name);
    });
</script>
@endsection